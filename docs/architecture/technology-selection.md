...
created: 2026-08-20
updated: 2026-08-27
links: [RFC-0007, ADR-0001, ADR-0002, ADR-0008, ROADMAP-0001, REVIEW-0009]
---
...
| G2：稳定单 Agent CLI | 单 Agent lifecycle、Effect/Evidence gate、Workspace/Sandbox policy 和 CLI command admission | 默认 Rust reference Provider/Tool/Hook；具体 Requirement 可批准受控外部 adapter | 外部组件不能构造 Event、Capability、Budget 或终态；不因 SDK 便利提前拆服务 |
| G3：受控 Multi-Agent | Plan/DAG、Lease/Heartbeat、取消与预算传播、Workspace ownership、合并和最终验证 | 隔离的本地或远程 Agent Worker，语言和产品不限 | Worker 不共享权威数据库或可写 Workspace，不自行判定 Task 完成 |
