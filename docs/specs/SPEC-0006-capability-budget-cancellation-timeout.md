...
- `CapabilityCommand` 支持 issue/delegate/revoke；`CancellationRequestCommandV1` 支持 Run/Task/operation target；`CancellationAckCommandV1`、`CallbackSettlementCommandV1` 都必须携带 Kernel-private opaque operation lease并匹配 producer/operation/reservation/callback namespace；`TimeoutRecoveryCommandV1` 只能由 Kernel recovery authority 从persisted `TimeoutKeyV1`、可信Clock sample与冻结usage-evidence observation确定性构造，同时固定command/event ID与canonical fingerprint；`RefundCommand` 固定 owner authority、原 settlement和修正向量。
...
`TimeoutKeyV1` 是reserve event固定的闭合identity preimage，包含schema/recovery contract revision、完整isolation scope与control stream、operation/reservation ID、absolute deadline、timeout/clock policy revision、source SchemaSet/limits digest及trusted operation/meter contract ref。它不含current Clock sample或可变usage observation。Kernel验证该key逐字段等于persisted operation，caller不能提供替代key。

`TimeoutRecoveryCommandV1` 固定`timeout_key`、`decision_clock_sample`、`usage_evidence_fingerprint`及派生的`event_id`/`command_fingerprint`。fingerprint是domain-separated canonical JSON preimage `pareto.runtime-timeout-recovery.command.v1` 加前三项的SHA-256；`event_id`为`event_`加同一lowercase hex digest。usage evidence必须在构造时冻结为Kernel-meter verified snapshot或显式`unknown`，不得在retry时重取。authority handle不进入wire fingerprint，但只有Kernel recovery authority能调用constructor。

`reconcile_operation_timeout_v1(recovery_authority, operation_id, clock_sample)`是显式Kernel-private authority-bearing入口，不能由plugin、Gate、adapter或普通principal构造。它可由live poll、callback admission或数据库reopen后的显式recovery调用；首片没有background scanner。命令在`BEGIN IMMEDIATE`内按固定优先级处理：1）integrity/isolation/source/expected TimeoutKey；2）若event ID已存在，canonical fingerprint/bytes exact相同返回原`AlreadyApplied`，同ID异fingerprint/bytes返回`idempotency_conflict`；3）fold后operation已有terminal且event ID不同，返回既有terminal no-op，不append、不audit、不核算；4）operation仍`reserved`时才判Clock。deadline前返回`not_due`，不append且不消费event ID或fingerprint，后来新Clock sample可派生新ID；恰好/超过deadline则用command的同一event ID追加唯一`timed_out` settlement。若没有可验证的Kernel meter evidence则对全部trusted reservation使用`unknown`保守consume；有verified partial meter时consume冻结actual并release差额。提交后响应丢失必须以缓存的exact command bytes重试并得到原结果；若进程已丢失该command，reopen用新sample派生不同ID并由步骤3安全取得既有terminal。reopen recovery必须枚举Projection中的pending operation逐个调用该入口，禁止自动release或reexecute。
...
| AC-08 | Core idempotency/security | provider report不权威；producer/lease/evidence负例；callback/refund/timeout recovery exact retry、same-ID mutation、commit-response-loss无重复核算 | `python scripts/assert_cargo_test_filter.py pareto-kernel runtime_control::usage_authority`; `python scripts/assert_cargo_test_filter.py pareto-kernel runtime_control::callback_authority`; `python scripts/assert_cargo_test_filter.py pareto-kernel runtime_control::idempotency` |
...
| AC-11 | Focused deterministic time/recovery | FakeClock deadline前/恰好/后；TimeoutKey/fingerprint/event ID golden；not_due不消费ID；无callbacktimeout；commit-response-loss exact retry；重启新epoch/新ID读既有terminal；wall regression/unknown结算；无sleep | `python scripts/assert_cargo_test_filter.py pareto-kernel runtime_control::deadline`; `python scripts/assert_cargo_test_filter.py pareto-kernel runtime_control::timeout_recovery`; `python scripts/check_req0007_scope.py` |
| AC-12 | Core concurrency/model | completion/cancel/timeout recovery/callback竞争、lifecycle transition/reserve竞争、deadline equality；same-ID mutation优先于terminal、different-ID晚到terminal no-op；bounded model至多一terminal | `python scripts/assert_cargo_test_filter.py pareto-kernel runtime_control::terminal_race`; `python scripts/assert_cargo_test_filter.py pareto-kernel runtime_control::lifecycle_reserve_race`; `python scripts/assert_cargo_test_filter.py pareto-kernel runtime_control::model_sequences` |
