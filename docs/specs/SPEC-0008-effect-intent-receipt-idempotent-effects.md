...
created: 2026-08-30
updated: 2026-08-30
links: [REQ-0009, EPIC-0002, REQ-0004, REQ-0007, REQ-0008, RFC-0009, RFC-0002, RFC-0003, RFC-0004, RFC-0005, RFC-0006, RFC-0008, ADR-0003, ADR-0004, ADR-0005, ADR-0006, ADR-0007, ADR-0009, ARCH-0002, ARCH-0003]
---
...
- `EffectRegistryRevisionV1`按effect kind排序唯一，registration固定effect/adapter/producer/operation-contract revisions、request/receipt Schema、external idempotency class、unknown/reconciliation/redaction policy、limits和Fake handler compatibility。
...
同Effect ID/exact request retry返回原状态且不重复reserve/claim/dispatch/settle；same key异request/subject/Task/contract/deadline为`idempotency_conflict`。跨scope/kind/revision不互相命中或泄漏。首片每Effect只有attempt 1，但Attempt ID/外部key token仍由Kernel稳定派生，为后续版本留下闭合身份而不授权重试。
...
`OperationReservedPayloadV1`和`OperationSettledPayloadV1`在新SchemaSet增加可选`effect_pair`，与既有`hook_pair`互斥。Effect pair固定pair ID/kind/fingerprint、operation/reservation/effect/attempt、control/effect counterpart event IDs及双方prepared bytes digest。
...
dispatch claim在独立writer transaction重新fold lifecycle/control/effect，验证live mode、registry、reserved operation、取消/deadline和unclaimed attempt。commit成功才生成crate-private `EffectDispatchLease`并由Kernel-owned orchestration直接调用Fake executor。already-claimed exact retry不再发lease或调用executor；crash-between-claim-and-call因此进入对账。

Fake executor返回`EffectReceiptObservationV1`或边界故障观察。Kernel先执行bounded decode，再验证producer/adapter、lease/process epoch、scope/effect/attempt/external key/request digest、Receipt Schema、Clock与Kernel meter。错误producer或跨域输入不写目标；已认证producer的invalid/oversized结果形成脱敏reject并按unknown terminal pair结清。Receipt仅保存safe digest、闭合outcome、bounded summary、usage observation与limitations，不保存敏感payload/秘密/路径/SQL/raw error。
...
Effect Projection绑定store/full scope/owner/stream/cursor、Manifest source SchemaSet/limits、registry/config/policy、reducer/output/history digest，按Effect ID规范排序。pure reducer无Clock、I/O、executor或mutable cache。

terminal source Run可从完整Effect range和recording policy构建Boundary Inventory：applied→Received，not-applied→Failed或Cancelled，partial/unknown→Failed with reconciliation-required reason，空Effect→显式空inventory。终态后late/reconciliation facts只生成新Boundary Reconciliation lineage，不覆盖inventory。

Recorded replay验证derived Manifest精确固定source inventory，读取source Effect stream重建并核对Projection/inventory；API不接收executor、writer、Clock、lease、reserve/settlement/reconciliation authority。Simulated/Reexecute Effect入口拒绝。
...
| API/schema | Manifest semantic validation现以major 1/2选择闭合roles，schema hardener固定Hook role；新增v3和Effect roles不能改变v1/v2字节 | `validation.rs::validate_run_manifest`、`schema.rs::harden_schema`、retained `schemas/sets`; golden/old/current-substitution tests |
...
| Failure/recovery | crash可落在Intent前后、claim前后、executor应用后、terminal pair前后；unknown必须对账而非release/reexecute | fault injection矩阵、reopen Projection、exact response-loss retry、Fake query reconciliation |
...
| Compatibility/migration | 新content-address SchemaSet与Manifest v3；SQLite保持v2，old Run/reducers/SchemaSets byte-identical | schema generator byte diff、retained set hashes、DB `user_version=2`/DDL/checksum/snapshot regression；发现DB v3需求即停 |
...
- Boundary Inventory finalizer只派生事实；Evidence Gate、artifact persistence、CLI和reexecute/simulated仍属后续Requirement。
...
实现发布一个新SchemaSet，不修改历史目录。Manifest v3基于v2添加Effect pin；protocol runtime dispatch必须按exact `schema_ref.major`选择roles/optional fields，禁止用一个current struct语义接受旧bytes。新增Effect event binding不能改变RunTask/RuntimeControl/Hook retained source keys、output digests或Snapshot bytes。
...
|---|---|---|---|
| AC-01 | Focused protocol/contract | Effect request/registry/Intent闭合字段、排序、unknown field/version/key digest | `cargo test -p pareto-protocol --test protocol_contract effect_contract --offline -- --exact` |
...
| AC-05 | Focused security | lease跨attempt/Run/Task/Actor/epoch复用、already-claimed retry不发lease | `cargo test -p pareto-kernel event_store::effect_runtime::dispatch_lease --offline -- --exact` |
...
| AC-10 | Focused recovery | Intent/claim/return/terminal各crash点，reopen与response loss不盲dispatch | `cargo test -p pareto-kernel event_store::effect_runtime::crash_recovery --offline -- --exact` |
...
| AC-17 | Focused recovery/projection | reopen Projection identity/digest；missing first/gap/row drift/alternate reducer拒绝 | `cargo test -p pareto-kernel event_store::effect_runtime::projection_recovery --offline -- --exact` |
| AC-18 | Focused replay/E2E | inventory exact lineage、Recorded零executor/append/reserve/settle；simulated/reexecute拒绝 | `cargo test -p pareto-kernel event_store::effect_runtime::recorded_replay --offline -- --exact` |
| AC-19 | Impacted compatibility/static | v1/v2/v3 Manifest与retained sets；SQLite v2/old reducer/snapshot bytes不变 | `cargo test -p pareto-kernel event_store::effect_runtime::compatibility --offline -- --exact`; schema generator byte check |
...
设计候选没有允许实施阶段自行决定的开放合同。独立架构评审必须重点挑战：Manifest v3是否是最小兼容演进；`hook_pair/effect_pair`互斥是否应改为通用boundary pair major；claim响应丢失的保守语义；Run/Task success guard；Boundary Inventory对partial/unknown的表达；以及首片不redispatch是否仍满足REQ-0009幂等结果。任何会改变持久Schema、authority、terminal、budget或replay的评审结论必须先修订REQ/RFC/SPEC并由同一Reviewer复审。
