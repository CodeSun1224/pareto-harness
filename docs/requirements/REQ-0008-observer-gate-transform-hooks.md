...
- AC-01：Hook 合同闭合区分 `observer | gate | transform`，每个注册固定稳定 logical ID、不可变 revision、配置摘要、类型、允许生命周期点、排序键、resource contract、input/output Schema 与 redaction policy；只有 Observer 固定选择 `warn-and-continue | fail-closed` failure policy，Gate 固定 fail closed，Transform 固定拒绝整个 proposal、无注册级策略选择；Run Manifest 固定完整有序注册集合及配置，运行中不得替换为 current/compatible revision。
...
- AC-03：注册和执行顺序由版本化闭合 phase 与排序规则决定：`before_proposal_admission` 固定 Transform → Gate → Observer，`before_authoritative_commit` 固定 Gate → Observer，`after_*` 只有 Observer；phase 内至少绑定显式 priority、stable logical ID 与 revision。每个 Transform 接收前一 Transform 的已验证输出；所有 Gate 接收同一最终 Transform 输出；Observer 只在 point 决定固定后接收该最终输入和决定的只读视图。相同输入、Manifest 和已记录历史产生相同逐 invocation lineage、顺序与决定。重复注册、排序键冲突、缺 revision/config 或 unknown major fail closed。
...
- AC-05：Observer 失败策略只能在注册时选择 `warn-and-continue | fail-closed`；失败不得隐式改变已经固定的 point 业务决定。Gate 一律 fail closed。Transform 失败、timeout 或非法输出固定拒绝整个 proposal，不得留下部分权威修改、返回原 proposal 继续执行或静默接受未转换输入。
...
- AC-09：每次 Hook invocation 使用 REQ-0007 trusted operation contract、Kernel meter、Run/Task/Actor/per-operation account 与同一 SQLite writer transaction 原子 reserve；Kernel-private pair command 必须固定 pair identity/fingerprint、双 stream expected cursor、两个 Event identity、zero/one/two-existing 判定与 rollback：zero 才原子写两者，two 仅 exact pair retry成功，one 一律视为损坏且不得补写或调用 Hook。proposal 自报用量不能降低上界，任何维度不足全部拒绝且不调用 Hook，并发调用不得超卖。
...
- AC-12：完成、取消、timeout、Kernel 重启与 response loss 由同一 writer serialization、稳定 terminal pair command 和双 stream 原子提交决定唯一 terminal；Hook operation 不得先经单 stream Runtime Control terminal 路径结清，再补写 Hook terminal。终态后的 late、duplicate、retry、out-of-order Hook 结果不得改变最终 Hook decision、权威状态、lifecycle、budget或Effect，只可形成绑定原 invocation 的脱敏审计事实。
...
- AC-14：Hook registration、point-start/input-lineage、invocation、attempt、result/decision、skip、point-finalization、late/rejected audit 由版本化 Event 表达并按连续 exact validated Hook stream pure fold；point finalization 绑定 source cursor、初始/最终 input digest、phase、有序 component identity 与最终决定。不得建立可被 Hook 写入的数据库、mutable decision table、共享 cache 或第二权威状态。最终权威事件仍只由 Kernel 在全部准入后提交。
...
- AC-20：测试覆盖 Hook 判定表、跨类型 phase/输入 lineage、稳定排序/组合/短路、默认拒绝、自我提权、Transform权威字段保护与中间失败、全隔离矩阵、Capability撤销/过期/收窄、budget reserve/settle与并发防超卖、atomic pair 的第二 append失败/rollback/exact-mutation retry/commit-response-loss/单边重封损坏、cancel/deadline/timeout/terminal pair race、hung recovery、late/duplicate/retry/out-of-order、crash/reopen、Event Store/Lifecycle/Projection/Snapshot/Replay回归、Recorded replay零执行/零核算、old/unknown版本、output injection/敏感日志/oversized output及bounded command/concurrency model；每个命名过滤命令必须证明非零命中。
