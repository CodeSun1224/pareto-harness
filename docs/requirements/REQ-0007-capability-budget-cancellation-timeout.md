...
- AC-11：每个受保护操作固定 absolute UTC deadline 与进程内 monotonic deadline。活进程以 monotonic clock 执行 timeout；持久化只保存 UTC deadline、timeout policy 与 clock-observation boundary，重启用注入 wall clock 判定已过期并建立新的 monotonic deadline。Kernel-private timeout/recovery command 必须能在没有 callback 时锁内重新 fold、权威产生 `timed_out` settlement，并按 verified 或 unknown 规则结清 reservation；该命令只能由 Kernel recovery authority 调用且语义幂等。测试只用 Fake Clock，不依赖真实 sleep。
- AC-12：完成只能在 deadline 之前且没有更早提交的有效 cancellation 时成功；恰好到 deadline 时 timeout 胜。deadline 前取消与完成、lifecycle pause/terminal 与 reserve/dispatch、timeout recovery 与 callback 并发均由同一 SQLite writer serialization 的首个合法 commit 决定；后到命令不得反转终态或在非运行 lifecycle 下制造新 dispatch。模型化并发测试必须接受可观察提交顺序中的唯一合法结果而不允许双终态。
...
- AC-18：稳定的 Kernel-private capability request、authorize/reserve、cancellation request/probe/ack、callback settlement、timeout recovery、refund 与 projection/replay 接口，为 REQ-0008、REQ-0009、REQ-0010、REQ-0011、REQ-0013、REQ-0014、REQ-0018 与 REQ-0033 留出版本化扩展点；这些接口必须携带冻结的 lifecycle guard、management/callback authority、trusted resource envelope 与 exact SchemaSet 约束。本需求不得实现 Hook framework、真实 Tool/Provider/外部 Effect、Sandbox、Agent Loop、Memory、Task DAG 或 WASM/WASI 隔离。
- AC-19：测试覆盖 Capability 判定表、默认拒绝/越权、delegation 收窄/禁止提权、撤销/到期、Run/Task lifecycle 准入全状态表与 transition/reserve 竞争、全隔离矩阵、取消 request/ack 授权矩阵、callback/settlement producer 绑定、可信 resource envelope 的低报/漏维度/超界负例、预算 reserve/consume/release/refund、并发防超卖、重试/失败/取消/未知用量、三级取消传播、无 callback timeout recovery、deadline/完成/取消竞态、迟到/重复/乱序结果、崩溃重开、Manifest/control SchemaSet exact 绑定、Event Store/Lifecycle/Projection/Snapshot/Replay 回归、Recorded replay 零执行/零重复核算、旧/未知 Schema 兼容及 bounded model/property 状态序列；每个过滤命令必须证明至少命中一个测试。
