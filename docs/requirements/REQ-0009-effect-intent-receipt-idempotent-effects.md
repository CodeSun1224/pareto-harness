...
created: 2026-08-30
updated: 2026-08-30
links: [EPIC-0002, REQ-0004, REQ-0007, REQ-0008, SPEC-0008, RFC-0009]
risk: high
...
- AC-01：Effect 合同闭合、版本化且不可变，至少固定 effect kind/revision、完整隔离域、Run/可选 Task、subject/executor、已批准 operation/reservation、规范化请求摘要、幂等键、deadline、重试/对账策略和 redaction policy；请求 payload、幂等键或 Receipt 本身都不等于 authority。
...
- AC-05：只有绑定 exact Intent/operation/reservation、用途受限且不可序列化伪造的单次 dispatch lease 能进入 Fake Effect executor；lease 的签发、领取、提交结果和失效规则可审计，不能被重放到另一请求、attempt、Run、Task、Actor 或 Workspace。
...
- AC-10：响应丢失或 crash/reopen 后，Kernel 从连续 exact Event history 恢复 pending/unknown Effect；exact command 重试幂等。只有 policy 明确声明幂等且复用同一外部幂等 identity，或可信 reconciliation 证明未执行时，才可授权再次 dispatch；否则进入 `reconciliation-required`。
...
- AC-13：取消、deadline 与 timeout 阻止新的 Intent 或尚未领取的 dispatch；已越过外部不可中断边界时只能记录 cancellation pending/unknown 并等待返回或 reconciliation，不能虚假确认外部效果停止。恰好 deadline 的竞争、callback 与 timeout/recovery 由同一 writer serialization 和稳定优先级产生唯一合法权威结果。
...
- AC-17：Effect Projection 在 crash/reopen 后恢复 Intent、attempt、Receipt、terminal、partial/unknown、reconciliation 与 budget binding；缺 first event、gap、非法顺序、双 terminal、错误 binding、unknown reader/reducer、row identity 漂移或 current-version substitution 均 fail closed。
- AC-18：Recorded replay 只读 source Run 已记录的 Effect 事实和 Projection，不创建 Intent、不签发 dispatch lease、不执行 Fake/真实 Effect、不查询外部系统、不 reserve/settle、不追加 Event。Simulated/reexecute 必须创建独立派生 Run并由后续 Requirement批准；本切片稳定拒绝其 Effect 执行入口。
- AC-19：发布 Effect/Event/Projection Schema 与内容地址 SchemaSet 时保留全部既有 SchemaSet、reader/reducer、SQLite v2 DDL/trigger和历史字节；只有 Manifest exact 固定 Effect-capable SchemaSet及effect registry/policy revision的 Run 能初始化。旧 Run 不后加 Effect、不静默升级。
- AC-20：测试覆盖默认拒绝、Intent-before-dispatch、幂等 exact retry/same-key mutation、全隔离矩阵、dispatch lease 复用、部分成功、响应丢失、commit-response-loss、执行前/后 crash、unknown outcome、cancel/deadline/timeout/receipt竞态、late/duplicate/out-of-order、reconciliation admission、预算原子结算、单边终态损坏、crash/reopen、old/unknown版本、Recorded replay零执行/零核算，以及 Event Store/Lifecycle/Runtime Control/Hook/Projection/Snapshot/Replay回归；每个命名过滤命令必须证明至少命中一个测试。
