...
### Historical design remediation

首轮独立设计评审固定 `05dd7ca` 并给出 `changes-requested`，0 Blocker、6 Major；这不是
最终验证结果，完整 findings 保存在 REVIEW-0006。修订期间的 `check_docs.py` 曾因
REVIEW-0001..0005 freshness 尚未由独立 Reviewer 前移而失败；F-004 identity 候选的 governance
18 tests 与 diff check 已通过，但组合门禁仍因同一 freshness 条件失败。随后同一独立 Reviewer
在 exact `a4e34785908207e622365250ae1466b85b4baecb` 实质复审全部修订，关闭所有 Major，
恢复 review freshness 并独立通过 docs/diff；最终可复算门禁结果以上方
`Independent design approval` 行为准。历史失败没有被重写为成功。
