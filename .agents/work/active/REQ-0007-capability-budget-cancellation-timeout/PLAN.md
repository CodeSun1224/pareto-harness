...
6. 实现Run/Task/operation cancellation request authority、probe/ack lease binding、FakeClock wall/monotonic deadline、显式Kernel timeout recovery和唯一terminal/lifecycle race；实现late/duplicate/out-of-order digest audit无状态/预算副作用。
...
- Cancel/time/race: filters `runtime_control::cancellation_authority`, `runtime_control::cancellation_propagation`, `runtime_control::interruptibility`, `runtime_control::deadline`, `runtime_control::timeout_recovery`, `runtime_control::terminal_race`, `runtime_control::model_sequences`
