...
TASK-03至08整改证据：新增唯一Kernel-owned `execute_hook_point`，生产路径完成Manifest/registry/schema/handler解析、point开始、atomic reserve/terminal、Transform→Gate→Observer、deny短路skip、finalization与Projection；deadline走Runtime Control timeout authority并记录late audit，invalid kind记录safe rejection audit。每条候选事实在同一writer事务中strict registry-aware fold，Projection同一MVCC horizon交叉验证control/Hook pair。`hook_runtime::` 37 passed，含4条生产纵向和validly-resealed负向历史。
