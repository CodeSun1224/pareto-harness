...
| Focused golden | `cargo run -p pareto-protocol --bin generate_schemas --offline -- schemas` followed by checked-in schema golden test | passed | 9 schemas + SchemaSet manifest/ref | Atomic directory publication and exact file-set/byte golden test prevent partial or stale output |
