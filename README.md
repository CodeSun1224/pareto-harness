...
可信内核的前两个纵向切片已交付：REQ-0003 提供版本化协议类型、JSON Schema 和合同测试；REQ-0004 提供 Kernel 私有、真实 SQLite 文件支持的 append-only Event Store，以及 migration、事务幂等、隔离、稳定分页与重启恢复测试。Run/Task 状态机、Projection、Replay executor 与 CLI 尚未实现。
...
`pre-alpha / protocol and SQLite event-store foundation delivered`. Claims in research documents are evidence-graded; target metrics are not presented as achieved results.
