...
可信内核的前三个纵向切片已交付：REQ-0003 提供版本化协议类型、JSON Schema 和合同测试；REQ-0004 提供 Kernel 私有、真实 SQLite 文件支持的 append-only Event Store；REQ-0005 提供完整 Run Manifest 首事件、Run/Task 状态机、owner-only authority、同事务 fold-and-append，以及重启恢复、幂等、并发、终态和隔离负测。Projection、Replay executor 与 CLI 尚未实现。
...
`pre-alpha / protocol, SQLite event-store, and lifecycle foundation delivered`. Claims in research documents are evidence-graded; target metrics are not presented as achieved results.
