...
项目目前进入可信内核的首个实现阶段。REQ-0003 提供不依赖 Runtime 服务的版本化协议类型、JSON Schema 和合同测试；Event Store、状态机、Replay executor 与 CLI 尚未实现。这里不会用空模块冒充完成度。
...
`pre-alpha / protocol foundation implementing`. Claims in research documents are evidence-graded; target metrics are not presented as achieved results.
